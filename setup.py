...
def make_samples_csv(path):
    ...
    for f in flist:
        if '.fastq.gz' in f and not ('_R1.fastq.gz' in f or '_R2.fastq.gz' in f):
            sample_id = f.split('.fastq.gz')[0]
            # this should simply return the full text to the left of '.fastq.gz'
